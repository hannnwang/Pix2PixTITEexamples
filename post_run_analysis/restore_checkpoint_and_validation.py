#Changed from restore_checkpoint_and_check_compare_runs.py, but works on validation set instead of test set.
...
#parser.add_argument("--cm_ins", required=True,
#                    help="colormap for total ssh") 
#parser.add_argument("--cm_cos", type=int, default=1,
#                    help="colormap for wave ssh")
...
NLAYERS=3 #***This might change
print('WARNING: nlayer in discriminator is fixed to be:')
print(NLAYERS)
os.chmod(PATH, 0O777)
...
validation_dataset=glob.glob(glob.escape(PATH)+'/validation/*.npy')#glob.escape is important if the PATH includes special characters like [ab].

#test_dataset = os.path.join(PATH,"test/",os.listdir(os.path.join(PATH, "test/")))
#print('test_dataset in list form:')
#print(test_dataset)
validation_dataset = load_image_test(validation_dataset)
#test_dataset = test_dataset.map(load_image_test)
validation_dataset = validation_dataset.batch(BATCH_SIZE)

nvalidation=len(os.listdir(PATH+'validation/'))
...
for ickpt in range(nckpt):
  ...
  cor_oneckpt=np.empty([nvalidation,OUTPUT_CHANNELS])
  L1_relative_oneckpt=np.empty([nvalidation,OUTPUT_CHANNELS])
  L2_relative_oneckpt=np.empty([nvalidation,OUTPUT_CHANNELS])
  ifig=0
  for inp, tar in validation_dataset: #running through all validation data
     ...
     for idim in range(OUTPUT_CHANNELS):
        ...
        SSH_tar=tar[0,:,:,idim]
        #with open(os.path.join(PATH+'validate_checkpoint/validateing_list_%i' % ifig +'_d%i.npz' %idim), 'wb') as f:
...
np.savez(PATH+'similarity_list_per_epoch_validation',epochs_recorded=epochs_recorded,cor_mean=cor_mean,cor_std=cor_std,
