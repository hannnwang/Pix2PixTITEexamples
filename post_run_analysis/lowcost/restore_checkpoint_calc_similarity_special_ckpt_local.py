...
#The codes mapping cmaps back to ssh are copied from Nico's code postprocess.py
...
#sizes of wptest_list, testkey_list and savekey_list should be the same!
wptest_list=[#'testing_wp60_3',
             #'testing_wp60_3',            
             #'testing_wp60_t_23_3',
             'testing_wp75_t_23_3',
             'testing_wp80_t_23_3',
...
savekey_list=['test_checkpoint_3p/','test_checkpoint_3p/','test_checkpoint_3p/']
...
print('WARNING: assuming inputs are square-shaped!')
...
for LAMBDA in lambda_list:
    for itest, wptest in enumerate(wptest_list):
        testkey=testkey_list[itest]
        savekey=savekey_list[itest]
        #runname='lambda_'+LAMBDA+'_nlayers_5_'+wptest+'_1000epochs_onechannel/'
        
        #runname='lambda_'+LAMBDA+'_nlayers_5_'+wptest+'_1000epochs_w_rotflipud/'
        #runname='lambda_'+LAMBDA+'_nlayers_5_'+wptest+'_1000epochs_256/'
        #runname='lambda_'+LAMBDA+'_nlayers_5_'+wptest+'_1000epochs_wocropping_w_resizing/'
        #runname='lambda_'+LAMBDA+'_nlayers_5_'+wptest+'_1000epochs_lof/'
        runname='lambda_'+LAMBDA+'_nlayers_5_'+wptest+'_700epochs_w_rotflipud/'

        PATH='C:/Users/han/Desktop/Pix2Pix/scratch_outputs/'+runname
        ...
        #For wp* runs:
        #test_dataset_list=glob.glob('C:/Users/han/Desktop/Pix2Pix/combined/'+testkey)#glob.escape is important if the PATH includes special characters like [ab].
        
        
        test_dataset_list = glob.glob(glob.escape(PATH)+'/test/'+testkey)
        ...
        ntest=len(test_dataset_list)
        
        

    
        if  os.path.exists(PATH+savekey)==False:
           os.mkdir(PATH+savekey) 
        #else:
        #   shutil.rmtree(PATH+'test_checkpoint/')
        #   os.mkdir(PATH+'test_checkpoint/') 
        
           
        for fakeiter in [1]: #If we are only checking the latest checkpoint
          filename=os.path.join(checkpoint_dir,ckptname)
          checkpoint.restore(filename).assert_consumed
          #=np.array(0)
          cor_ssh=np.empty([ntest,1])
          L1_absolute_ssh=np.empty([ntest,1])
          L2_absolute_ssh=np.empty([ntest,1])
          L1_relative_ssh=np.empty([ntest,1])
          L2_relative_ssh=np.empty([ntest,1])
          ifig=0
          for inp, tar in test_dataset: #running through all test data
             ...
             for idim in range(1):
                ...
                SSH_tar=tar[0,:,:,idim]
                with open(os.path.join(PATH+savekey+ckptname+'_'+filename +'.npz'), 'wb') as f:
                 np.savez(f, SSH_raw=SSH_raw,SSH_fak=SSH_fak,SSH_tar=SSH_tar)    
              
                cor_ssh[ifig,idim]=similarity(SSH_tar, SSH_fak, "Correlation")
                L1_absolute_ssh[ifig,idim]=similarity(SSH_tar, SSH_fak, "L1")
                L2_absolute_ssh[ifig,idim]=similarity(SSH_tar, SSH_fak, "L2")
                L1_relative_ssh[ifig,idim]=similarity(SSH_tar, SSH_fak, "L1_relative")
                L2_relative_ssh[ifig,idim]=similarity(SSH_tar, SSH_fak, "L2_relative")
        
             ifig=ifig+1
            
          print('mean correlation:')
          print(np.mean(cor_ssh[:,0]))
          np.savez(PATH+savekey+'similarity_list_'+ckptname,cor_ssh=cor_ssh, L1_relative_ssh=L1_relative_ssh, L2_relative_ssh=L2_relative_ssh, L1_absolute_ssh=L1_absolute_ssh,L2_absolute_ssh=L2_absolute_ssh)
